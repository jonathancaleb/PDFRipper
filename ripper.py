...
class Example(tk.Frame):
    ...
    def process_pdf(self):
        ...
        if self.file is not None:
            popup_value = self.popup.get()
            try:
                to_delete = auxilary_scripts.delete_pages(popup_value.split(';'), self.file.name)
            except AttributeError:
                to_delete = auxilary_scripts.delete_pages([], self.file.name)
                auxilary_scripts.create_pdf(self.file.name, to_delete)
        else:
            # Handle the case where self.file is None
            # You might want to raise an exception or provide a default value
            raise ValueError("self.file is None. You must provide a valid file.")
    ...
    def pdf_to_images(self):
        ...
        try:
            ...
            auxilary_scripts.pdf_to_pil('temp.pdf', folder, self.file.name.split('/')[-1].split('.')[0])
        except TypeError:
            ...
            if len(self.array_crop_coords[i]) > 0:
                ...
                for j in range(len(self.array_crop_coords[i])):
                    ...
                    if self.array_crop_coords_dict[i][j]['name'].endswith('_email'):
                        texto = auxilary_scripts.pytesseract.image_to_string(cropped_image, lang = 'eng', config = '--psm 7')
                    if self.array_crop_coords_dict[i][j]['name'].endswith('_ML'):
                        texto = auxilary_scripts.pytesseract.image_to_string(cropped_image, lang = MY_LANG, config = '--psm 6') #This one seems to be the one that works best for multiple lines
                    else:
                        texto = auxilary_scripts.pytesseract.image_to_string(cropped_image, lang = MY_LANG, config = '--psm 7')
